...
const Visits = () => {
  ...
  const { data: visits, isLoading, isError, refetch } = useGetVisitsQuery({ id: userId }, { refetchOnMountOrArgChange: true })
  const navigate = useNavigate();
  const [deleteVisit, { isLoading:loading }] = useDeleteVisitMutation();

  const handleDelete = (id:string) => {
    // const id = visit._id
    const deleted = deleteVisit(id);
    refetch()
  }
