...
import { useGetMessagesQuery, useUpdateMessageMutation, useDeleteMessageMutation } from "../../store/slices/api/apiEndpoints";
import SmartToyIcon from '@mui/icons-material/SmartToy';
import { useState } from "react";

const Messages = () => {
  const user = useSelector(selectCurrentUser);
  const { data: messages, isLoading, isError, Error, refech } = useGetMessagesQuery(user.id || '')
  ...
  const [currentMessageData, setCurrentMessageData] = useState(isLoading?{}:userMessages[0])
  ...
  const messageDivs = userMessages?.map((message, id) => {
    ...
    return (
      <div id={message._id} onClick={() => setCurrentMessageData(message)} key={id}
        className={`${currentMessageData._id === message._id && 'bg-slate-400'} flex bg-white p-2 gap-1 shadow-sm shadow-gray-100 cursor-pointer hover:bg-gray-300 rounded-md mb-2`}>
        <SmartToyIcon />
        <p>{message.title}</p>
